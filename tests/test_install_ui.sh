...
[[ "$common_body" != *"collect_port53_confirmation()"* ]] || fail "installer must not prompt for port 53"
[[ "$install_body" != *'run_install_step "释放 DNS 端口"'* ]] || fail "port 53 must not be an install stage"
