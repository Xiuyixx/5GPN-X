...
Assert-Contains $install 'ip saddr 10.100.0.0/16 tcp dport { 80, 443 } accept' 'nft TCP reverse proxy po0 client allow'
Assert-Contains $install 'ip saddr 10.100.0.0/16 udp dport 443 accept' 'nft UDP reverse proxy po0 client allow'
Assert-Contains $install 'iptables -A INPUT -s 10.100.0.0/16 -p tcp -m multiport --dports 80,443 -j ACCEPT' 'iptables TCP reverse proxy po0 client allow'
Assert-Contains $install 'iptables -A INPUT -s 10.100.0.0/16 -p udp --dport 443 -j ACCEPT' 'iptables UDP reverse proxy po0 client allow'
...
Assert-Contains $install 'Firewall configured (reverse proxy whitelist: 172.22.0.0/16, 10.100.0.0/16)' 'firewall status message'
Assert-Contains $readme '172.22.0.0/16' 'README documents reverse proxy whitelist'
Assert-Contains $readme '10.100.0.0/16' 'README documents the 10.100.0.0/16 client network'
