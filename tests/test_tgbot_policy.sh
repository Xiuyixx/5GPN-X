...
bot_body="$(cat "${bot}")"
main_menu_body="$(sed -n '/^def main_menu():/,/^$/p' "${bot}")"
dot_menu_body="$(sed -n '/^def dot_menu():/,/^$/p' "${bot}")"
services_menu_body="$(sed -n '/^def services_menu(prefix):/,/^$/p' "${bot}")"
...
[[ "${main_menu_body}" == *'"🛠️ 5GPN 管理", "callback_data": "menu:dot"'* ]] || fail "main menu must expose 5GPN management with the new icon"
[[ "${main_menu_body}" != *'重启服务'* && "${main_menu_body}" != *'日志'* ]] || fail "restart and logs must leave the main menu"
[[ "${dot_menu_body}" == *'"♻️ 重启服务", "callback_data": "act:restart"'* ]] || fail "5GPN management must contain restart"
[[ "${dot_menu_body}" == *'"📜 日志", "callback_data": "menu:logs"'* ]] || fail "5GPN management must contain logs"
[[ "${services_menu_body}" == *'"callback_data": "menu:dot"'* ]] || fail "logs service list must return to 5GPN management"
[[ "${bot_body}" == *'"🛠️ <b>5GPN 管理</b>"'* ]] || fail "5GPN management page must use the new title"
[[ "${bot_body}" == *'edit_async(cb, op_renew_cert, back_kb("menu:dot"))'* ]] || fail "certificate renewal must return to 5GPN management"
[[ "${bot_body}" == *'edit_async(cb, op_restart_services, back_kb("menu:dot"))'* ]] || fail "restart result must return to 5GPN management"
...
[[ "${bot_body}" == *'def dot_menu('* ]] || fail "tgbot.py must expose the 5GPN management submenu"
...
[[ "${install_body}" == *'info "DNS: remote=$REMOTE_DNS local=$LOCAL_DNS"'* ]] || fail "install.sh must report remote/local DNS setup cleanly"
