...
[[ "${install_body}" == *'国际 DNS remote [Cloudflare DoH, Google UDP fallback]'* ]] || fail "install.sh must present remote DNS setup wording"
[[ "${install_body}" == *'国内 DNS local [AliDNS DoH, DNSPod UDP fallback]'* ]] || fail "install.sh must present local DNS setup wording"
