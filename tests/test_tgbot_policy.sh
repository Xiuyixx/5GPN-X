...
[[ "${bot_body}" == *'当前 DNS：<code>%s</code>'* ]] || fail "tgbot.py DoT status must show one current DNS value when values match"
...
[[ "${bot_body}" == *'def next_auto_exit_name('* ]] || fail "tgbot.py must auto-assign exit names for direct pasted links"
...
[[ "${bot_body}" == *'"[Interface]" in config and "[Peer]" in config'* ]] || fail "add-exit flow must keep WireGuard paste support"
[[ "${bot_body}" == *'添加出口：直接粘贴一条节点链接即可。'* ]] || fail "add-exit prompt must tell users to paste one link directly"
