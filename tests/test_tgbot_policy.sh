...
[[ "${bot_body}" == *'节点消息读取后会自动删除'* ]] || fail "add-exit prompt must disclose automatic credential message deletion"
...
[[ "${bot_body}" == *'items, err = parse_add_exit_inputs('* ]] || fail "add-exit flow must use batch-capable parsing"
