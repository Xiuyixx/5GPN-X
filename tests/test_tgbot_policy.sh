...
[[ "${bot_body}" == *'最多发送两行：第一行 remote 国际 DNS，第二行 local 国内 DNS。'* ]] || fail "tgbot.py DNS flow must accept remote/local DNS input"
[[ "${bot_body}" == *'remote 国际 DNS：<code>%s</code>'* ]] || fail "tgbot.py DoT status must show remote DNS"
[[ "${bot_body}" == *'local 国内 DNS：<code>%s</code>'* ]] || fail "tgbot.py DoT status must show local DNS"
