...
main_install() {
    ...
    run_install_step "部署代理与 DNS 服务" install_stage_services
    run_install_step "配置分流与系统网络" install_stage_network
    run_install_step "生成客户端配置" install_stage_client
    run_install_step "启动并检查服务" install_stage_start

    collect_tgbot_settings
    run_install_step "配置自动维护" install_stage_maintenance
    rm -f "$INSTALL_STATE"
    trap - EXIT

    printf '\n%b✓ 安装完成%b\n' "$GREEN" "$NC"
    cat <<EOF
  Android 私人 DNS: ${DOMAIN}
  iOS 描述文件:     http://${DOMAIN}:${IOS_PROFILE_PORT}/ios-dot.mobileconfig
  代理入口:         ${PUBLIC_IP}（TCP 80/443，UDP 443，DNS 53）
  管理命令:         $0 --status
  安装日志:         ${INSTALL_LOG}
EOF
    echo ""
