...
服务器端透明代理网关：dnsdist DoT + SNI/QUIC 透明代理 + 可切换多协议出口。客户端只需把 DNS/DoT 指向服务器域名，无需安装任何代理客户端。
...
- AAAA 返回 NOERROR/NODATA（IPv4-only 设计，避免 IPv6 绕过网关）。
...
- DNS + DoT：TCP/UDP 53 与 TCP 853，来源网段策略、ECS 覆盖、QPS 限制。
...
sudo ./install.sh --update-rules        # 更新 GFWList/ChinaList 并重载 dnsdist
...
REMOTE_DNS="1.1.1.1,8.8.8.8"    # 国际/代理侧解析（dnsdist remote 池 + sniproxy）
LOCAL_DNS="223.5.5.5,119.29.29.29"  # ChinaList 国内解析
...
**为什么不返回 IPv6？** IPv4-only 透明代理设计；AAAA 返回 NOERROR/NODATA，避免客户端走 IPv6 绕过网关或连接失败。

**为什么国内网站直连？** ChinaList 域名转发到本机国内 DNS 竞速代理并携带国内 ECS，获得更适合大陆访问的结果。
...
**如何查看日志？** 相关服务：`dnsdist`、`sniproxy`、`wa-shim`、`quic-proxy`、`china-dns-race-proxy`、`proxy-gateway-tgbot`、`proxy-gateway-mihomo@*`，用 `systemctl status` / `journalctl -u <服务> -f` 查看。
