...
- 上述伪造和 AAAA 抑制仅应用于 `172.22.0.0/16`、`10.100.0.0/16` 来源；Wi-Fi、公网及其他来源始终返回正常的 A/AAAA 解析结果。
...
REMOTE_DNS="1.1.1.1,8.8.8.8"    # 国际/代理侧解析（mosdns remote 池 + sniproxy）
...
**为什么内网客户端不返回 IPv6？** 透明代理路径按 IPv4 设计；只有 `172.22.0.0/16`、`10.100.0.0/16` 来源的 AAAA 返回 NOERROR/NODATA。Wi-Fi、公网及其他来源正常返回 IPv6。
